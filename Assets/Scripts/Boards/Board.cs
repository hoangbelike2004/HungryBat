...
public class Board
{
    ...
    public eBonusType GetBonusItem(List<Cell> matches)
    {
        if (matches.Count == 4)
        {
            ...
            var listvt = matches.Where(x => x.BoardY == matches[0].BoardY).ToList();
            if (listvt.Count == matches.Count)
            {
    ...
    internal List<Cell> FindFirstMatch()
    {
        ...
        for (int x = 0; x < boardSizeX; x++)
        {
            for (int y = 0; y < boardSizeY; y++)
            {
                ...
                List<Cell> listhor = GetHorizontalMatches(cell);
